...
const int16_t paddle_h = 30;
const int16_t paddle_w = 8;   // was: 4

const int16_t lpaddle_x = 0;
const int16_t rpaddle_x = w - paddle_w;

const int16_t ball_w = 12;    // was: 6
const int16_t ball_h = 12;    // was: 6
...
static int16_t lpaddle_y;
static int16_t rpaddle_y;

static int16_t lpaddle_d;
static int16_t rpaddle_d;

static int16_t target_y;

static int16_t ball_x;
static int16_t ball_y;
static int16_t oldball_x;
static int16_t oldball_y;
...
void ball()
{
  ...
  if (    (ball_dx == -1)
       && (ball_x == paddle_w)
       && ((ball_y + ball_h) >= lpaddle_y)
       && (ball_y <= (lpaddle_y + paddle_h)))
  {
    ball_dx = -ball_dx;
    ...
    calc_target_y(); 
            ...
            && ((ball_x + ball_w) == (w - paddle_w))
            && ((ball_y + ball_h) >= rpaddle_y)
  ...
  else if (    ((ball_dx == 1) && (ball_x >= w))
            || ((ball_dx == -1) && (ball_x + ball_w < 0)) )
  {
    dly = 5;
  }
